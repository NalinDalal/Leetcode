...
public:
  vector<int> finalPrices(vector<int>& prices) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = i + 1; j < n; j++) {
        if (prices[j] <= prices[i]) { // Find the first discount
...
// Time Complexity: 𝑂(𝑛^2) due to the nested loops.
// Space Complexity: 𝑂(1) additional space (besides the result array).
// 100%
