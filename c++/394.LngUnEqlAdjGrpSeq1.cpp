...
..., n - 1] denoted as [i0, i1, ..., ik-1], such that groups[ij] != groups[ij+1]
...
be selected is ["a","b","d"] because groups[0] != groups[1] and groups[1] !=
groups[3]. It can be shown that the length of the longest subsequence of indices
...
/* find the longest subsequence in groups where adjacent elements are different
use dp, dp[i] represent length of the longest valid subsequence ending at index
i. element j b/f i satisfies grp[i]!=grp[j], then append ith str a/f jth->
dp[i]=dp[j]+1

`if(groups[i]!=groups[j]) dp[i]=max(dp[i],dp[j]+1)`
calculate longest subsequence ending with i
use prev[i] to record the index j of the previous element in the longest
subsequence for index i.
...
// sub:
// https://leetcode.com/problems/longest-unequal-adjacent-groups-subsequence-i/submissions/1634682380/?envType=daily-question&envId=2025-05-15
