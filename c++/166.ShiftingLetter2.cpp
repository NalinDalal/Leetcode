...
/* Approach:
1. get size of str;declare array of size n, all element=0
2. for loop to traverse array `shifts`
    - If direction is forward (1):
    - - Increment at the start index to indicate a forward shift.
    - - Decrement at the end+1 index to stop the effect of the shift.
    - If direction is backward (0):
    - - Decrement at the start index to indicate a backward shift.
    - - Increment at the end+1 index to stop the effect of the shift.
3. return result
    4. iterate over string `s`
                           - Update cumulative shifts, keeping within the
alphabet range.
  ...
  string shiftingLetters(string s, vector<vector<int>>& shifts) {
    ...
    for (auto shift : shifts) {
      if (shift[2] == 1) {     // If direction is forward (1):
        ...
        if (shift[1] + 1 < n) {
          diffArray[shift[1] + 1]--; // Decrement at the end+1 index to stop the
                                     // effect of the shift.
        }
        ...
        if (shift[1] + 1 < n) {
          diffArray[shift[1] + 1]++; // Increment at the end+1 index to stop the
                                     // effect of the shift.
        }
      }
    }
    ...
    for (int i = 0; i < s.size(); i++) {
      numberOfShifts = (numberOfShifts + diffArray[i]) %
                       26; // Update cumulative shifts, keeping within
                           // the alphabet range.
      if (numberOfShifts < 0)
        numberOfShifts += 26; // Ensure `numberOfShifts` is non-negative.
...
// Time: O(n)
// Space: O(n)
// https://leetcode.com/problems/shifting-letters-ii/submissions/1498753071/?envType=daily-question&envId=2025-01-05
