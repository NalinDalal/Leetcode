...
// for each iteration find min of array and replace it with multipler*min, till
// k times
/*class Solution {
public:
...
public:
  vector<int> getFinalState(vector<int>& nums, int k, int multiplier) {
    while (k--) {
      // Find the iterator to the minimum element in the array
      auto minIt = std::min_element(nums.begin(), nums.end());

      // Multiply the minimum element by the multiplier and update it
