...
for which there exist two adjacent subarrays of length k each, such that both
...
/*store length of current subarray which is strictly increasing in `cnt`
`prev` stores length of the previous strictly increasing subarray
...
for(int num:nums){
    ...
    previous subarray corresponds to precnt, and the current subarray
corresponds to cnt k=min(precnt,cnt)

By continuously updating the maximum value of k according to these two cases, we
can obtain the final answer.
```
*/
...
// sub:
// https://leetcode.com/problems/adjacent-increasing-subarrays-detection-ii/submissions/1802529051/?envType=daily-question&envId=2025-10-15
// sol:
// https://leetcode.com/problems/adjacent-increasing-subarrays-detection-ii/solutions/7277592/3350-adjacent-increasing-subarrays-detec-7y6u/
