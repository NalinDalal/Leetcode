...
accepted, you need to do the following things: Change the array nums such that
...
// Algo:
/* 1. To remove all occurences of val in nums, while maintaining order
2. initialise a pointer k to keep track of elem in nums which valid
3. If the current element nums[i] is not equal to val, assign nums[k] = nums[i]
and increment k.
4. skip element = val
5. return k
*/
